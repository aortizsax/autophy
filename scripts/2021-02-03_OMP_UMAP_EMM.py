...
# =============================================================================
    ...
    for i in range(0,len(taxons)):
        taxonsLabels.append(taxons[i]+"|Subfamliy|"+str(clf.predict(X)[i]))
        #print(clf.predict(X)[i],taxons[i],BPcolors[clf.predict(X)[i]%len(BPcolors)])
        #for coloring tree
        catTax[int(clf.predict(X)[i])].append(taxons[i]+"|Subfamliy|"+str(clf.predict(X)[i]))
        #for tree label replacement
        repDict[taxons[i]] = taxons[i]+"|Subfamliy|"+str(clf.predict(X)[i])
