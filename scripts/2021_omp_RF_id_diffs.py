...
TREEPATH ="../data/2021-01-25_BSH_filterd_MP.nwk"
FAM = 'BSH'
N = 2100
HEIGHT = 22

ALNPATH =  '../data/OMPaln/uniprot-gene_omp+reviewed_yes-8924.meg'
TREEPATH = "../data/2021-023-02_ML_OMP.nwk"
FAM = 'OMP'
N=1650
HEIGHT = 22
        ...
        for l in reader:
          ...
          #      gapsDict[]
            if len(l) ==0:
                ...
                if l.find('mega') == -1:
                    seqCount += 1
                    taxon = catDict[l.strip()[1:].replace('_','.').replace('(','').replace(')','').split('.')[0]]
...
for taxon, aln in alnDict.items():
    if type(taxon) == int or taxon == 'mega':
        ...
        #print(len(aln))
        taxon = taxon.strip().replace('_','.').replace("(",'').replace(")",'')
        print('>'+catDict[taxon.split('.')[0]],aln[int(alnImp[0])-5:int(alnImp[0])+5],
              ...
              aln[int(alnImp[5])-5:int(alnImp[5])+5],
              aln[int(alnImp[6])-5:int(alnImp[6])+5],
              aln[int(alnImp[7])-5:int(alnImp[7])+5],
              aln[int(alnImp[8])-5:int(alnImp[8])+5],
              aln[int(alnImp[9])-5:int(alnImp[9])+5],
              aln[int(alnImp[10])-5:int(alnImp[10])+5],
