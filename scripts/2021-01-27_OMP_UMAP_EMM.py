...
TREEPATH = "../data/BSHtree/2021-01-15_BSH_filterd_MP.nwk"
OUTTREE = "../data/2021-01-25_BSH_filterd_MP.nwk"
FAM = 'BSH'
HEIGHT = 22
...
# ##"2021_MP_OMP.nwk" 
# OUTTREE = "../data/2021-01-25_MP_OMP.nwk"
# FAM = 'OMP'
# HEIGHT = 22
# 
# =============================================================================
...
for i in range(0,len(taxons)):
    taxonsLabels.append(taxons[i].replace(' ','.').replace('(','..').replace(')','..'))
    #print(clf.predict(X)[i],taxons[i],BPcolors[clf.predict(X)[i]%len(BPcolors)]
    #for tree label replacement
    repDict[taxons[i]] = taxons[i].replace(' ','.').replace('(','..').replace(')','..')#.split(' ')[0].split('_')[0]
