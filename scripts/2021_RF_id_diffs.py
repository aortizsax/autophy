        ...
        for l in reader:
          ...
          #      gapsDict[]
            if len(l) ==0:
                ...
                if l.find('mega') == -1:
                    seqCount += 1
                    print(l.strip()[1:].replace('_','.').replace('(','').replace(')',''))
                    taxon = catDict[l.strip()[1:].replace('_','.').replace('(','').replace(')','').replace('|org|','|').replace('|org-type|Bacteria','')]
...
for taxon, aln in alnDict.items():
    if type(taxon) == int or taxon == 'mega':
        ...
        #print(len(aln))              
        info=['>'+catDict[taxon.strip().replace('_','.').replace('(','').replace(')','')].replace('.|org|','|').replace('|org-type|Bacteria','').replace('Streptococcus.faecium.','.').replace('.strain.DSM.17509./.CIP.109821./.100-23.','.').replace('.subsp..saprophyticus.','').replace('.strain.ATCC.BAA-472./.TX0016./.DO.',''),
        ...
        l.append(info)
        print('>'+catDict[taxon.strip().replace('_','.').replace('(','').replace(')','')],
              aln[int(alnImp[0])-5:int(alnImp[0])+5],
              aln[int(alnImp[1])-5:int(alnImp[1])+5],
              aln[int(alnImp[2])-5:int(alnImp[2])+5],
              aln[int(alnImp[3])-5:int(alnImp[3])+5],
