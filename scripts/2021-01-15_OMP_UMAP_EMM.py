...
n_Hs = [20,100]#[20,100,150,180,220,1000,2000,3000]
...
for n_H in n_Hs:
    ...
    plt.savefig('../data/EMMplots/2021-01-15_'+str(n_H)+'_GMMsweep_'+FAM+'_'+metric+'.png')
    plt.show()
    ...
    with plt.rc_context({'lines.linewidth': 3}):
        ...
        plt.savefig("../data/"+FAM+"colored/2021-01-15_"+str(n_H)+"_EMClust_"+FAM+"_"+metric+"_coloredtree.png")
        plt.show()

    #make target vector
    labs = []
    for ii in finalDf['target']:
        labs.append(int(repDict[ii].split('|')[-1]))#print(ii,repDict[ii].split('|')[-1])
    finalDf['labels'] = labs
    print(finalDf)
