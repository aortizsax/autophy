...
viz.fit(x, y)
viz.show()
fig.savefig('../data/'+FAM+'colored/2021-08-04_'+FAM+TREEID+'_logimportance.tiff',dpi=300)

model = LogisticRegression(multi_class="auto", solver="liblinear",random_state=1111).fit(x,y)
...
impdf = pd.DataFrame(impdict, columns = ['MSA Feature','Relative Importance'],index=index)
...
fig.savefig('../data/'+FAM+'colored/2021-08-04_'+FAM+TREEID+'_RFClassimportance.tiff',dpi=300)
fig.show()
