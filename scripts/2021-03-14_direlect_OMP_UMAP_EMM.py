...
for i in range(0,len(taxons)):
   ...
   # taxa = "|".join(taxa.split('|')[:-1]).split('...')[0].replace('|Bacteria|','|')#.split('..')[0]
...
strTree = re.sub("subsp",'.', strTree)
strTree = re.sub(".saprophyticus",'.', strTree)
