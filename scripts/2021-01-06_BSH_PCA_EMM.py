...
bic = np.array(bic)
BPcolors = ['navy','olive','teal','purple','gold','orange','salmon','red','cyan','tan']
...
for i, (mean, cov, color) in enumerate(zip(clf.means_, clf.covariances_,
    ...
    for tf in Y_==i:
        if tf == True:
            colorDict[color].append(taxons[j])
        j+=1
...
plt.savefig('../data/2021-01-06_GMMsweepBSH.png')
plt.show()
...
#     print(k,v,BPcolors[k%len(BPcolors)],Y_==k)#colorDict[BPcolors[k%len(BPcolors)]])
#     if len(v)==1:
...
for k,v in colorDict.items():
    print(k,v,Y_==k)#colorDict[BPcolors[k%len(BPcolors)]])
   # if len(catTax[])==1:
        mrca = BPtree.common_ancestor(catTax[k])#[i], v[i+1])
        mrca.color = k
    for i in range(len(v)-1):
        mp = BPtree.is_monophyletic(catTax[k])
        mrca = BPtree.common_ancestor(catTax[k])#[i], v[i+1]) 
        mrca.color = k
...
with plt.rc_context({'lines.linewidth': 3}):
    ...
    plt.savefig("../data/2021-01-06_EMClust_BSH_coloredtree.png")
    plt.show()
#print(xmlBPtree)
