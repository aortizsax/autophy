...
for n_H in n_Hs:
        ...
        while len(f) > 0:
            ...
            while mp:
                for adj_edg in parentedge.get_adjacent_edges():
                    print(
                        adj_edg.bipartition.leafset_as_newick_string(
                            tree.taxon_namespace
                        ).split(", (")[0]
                    )
                ...
                if check is True:
                    print(
                        "This edge is still monophyletic {} contains only elements of the list {}".format(
                            descend, f
                        )
                    )
    ...
    OUTF = (
        "../data/" + FAM + "colored/2022-04-21" + FAM + TREEID + "_coloredPASSONE.csv"
    )
    with open(OUTF, "w") as f:
        writer = csv.writer(f)
...
tree.ladderize()
Y_table = pd.read_csv(
    "../data/" + FAM + "colored/2022-04-21" + FAM + TREEID + "_colored_RF.csv",
    dtype="string",
)
...
plt.savefig(
    "../data/" + FAM + "colored/2022-04-21" + FAM + TREEID + "_subBAR_MPD.svg",
    format="svg",
)
plt.show()
...
rankD.plot.scatter(x=x, y=y, color=rankD["Color"], s=rankD["size"] * 75, figsize=(9, 8))
# =============================================================================
...
for k in rankD.iterrows():
    clust = k[1]["Clust"]
    color = k[1]["Color"]
    matplotlib.pyplot.annotate(k[1]["Clust"], (k[1][x], k[1][y]), color=color, size=18)
plt.title(FAM + " " + TREEID)
plt.xlabel("Log Odd Tree Mean Pairwise Distance")
plt.ylabel("Log Odd Parent Mean Pairwise Distance")

plt.savefig(
    "../data/" + FAM + "colored/2022-04-21" + FAM + TREEID + "_subpar_MPD_scatter.svg",
    format="svg",
)
plt.show()
...
plt.savefig(
    "../data/" + FAM + "colored/2022-04-21" + FAM + TREEID + "_sPASONEubBAR_MPD.svg",
    format="svg",
)
plt.show()
